...
class LeaguesResearch():
    ...
    def leagues_summary(self):
        ...
        for league_id in self.leagues_list:
            ...
            league_name = self.names_mapping.get(league_info.league_name, league_info.league_name)
            ...
            mvp_teams = standings.loc[standings.pure_points == standings['pure_points'].max(), 'team_name']
            mvp_names = standings.loc[standings.pure_points == standings['pure_points'].max(), 'user_name']
            league_summary = {
        ...
        summary_df['new_total_score'] = (summary_df['old_total_score'] + summary_df['new_score']).round(2)
        summary_df['new_score'] = summary_df['new_score'].round(2)

        summary_df['old_position'] = summary_df['old_total_score'].rank(method='min', ascending=False).astype(int)
        summary_df['new_position'] = summary_df['new_total_score'].rank(method='min', ascending=False).astype(int)
