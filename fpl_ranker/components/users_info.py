...
class UserInfo:
    def __init__(
        self, entry: int, players_info: PlayerInfo, name: str = None, team: str = None
    ):
    ...
    def _get_user_history(self):
        ...
        url = network_utils.url_join(personal_body, str(self.entry), personal_suffix)
        data = network_utils.safe_request(url)
        return data["current"]
    ...
    def _get_event(self, event_id: int):
        ...
        url = network_utils.url_join(self.event_body, event_id, "picks")
        self.event_storage[event_id] = network_utils.safe_request(url)
        return self.event_storage[event_id]
